  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // /lodash/api/. It is produced by the Go doc generator at deploy time,
        ...
        expect(resolved.pathname, `docs link should resolve under /lodash/`).toBe('/lodash/api/');
      } else {
...
test('docs tab renders the React API reference with a package listed', async ({ page }) => {
  ...
  // lodash is a single-package module (package lodash), so at least one package link is expected.
